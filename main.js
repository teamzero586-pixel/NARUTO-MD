...
//    set one during pairing, otherwise the default Naruto-MD branding ──
...
async function brandedReply(conn, from, mek, text) {
    ...
    // otherwise the default Naruto Mini Bot channel. Note: WhatsApp pulls a
...
// internal channel.
const CHANNEL_JID = config.CHANNEL_JID || '120363427395977020@newsletter';
...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [Naruto-MD] ${new Date().toISOString()}: ${message}`);
}
...
// ========== EXTRACT MESSAGE BODY (Naruto-MD Style) ==========
...
// ========== EXTRACT BUTTON ID (Naruto-MD Style) ==========
...
*© Powered by ${OWNER_NAME}*`;
    ...
    if (connection === 'close') {
        ...
        arslanLog(`Naruto-MD Pair error: ${err.message}`, 'error');
